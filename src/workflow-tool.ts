  ...
  return defineTool({
    ...
    get promptGuidelines() {
      return [
        ...
        "For workflow, for side-effecting fan-out that writes files (edits, generated code), prefer isolation:'worktree' on each agent: its edits are committed and merged back to the base repo only on success, so concurrent agents don't clobber each other and a failed/interrupted agent leaves the base tree untouched (safe to resume without double-applying).",
